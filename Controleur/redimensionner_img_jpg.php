...
$source = imagecreatefromjpeg(('../uploads/img/originaux/' . $nom_final . ''));// on stocke la photo originale dans une variable
...
if ($largeur_source < $hauteur_source) // si l'image source est au format portrait
	{
		...
		imagejpeg($destination_portrait, '../uploads/img/avatars/avatar_' . $resultat['id'] . '.jpg'); // on stocke la nouvelle image vers le chemin souhaité
	}else
	{
		...
		imagejpeg($destination_paysage, '../uploads/img/avatars/avatar_' . $resultat['id'] . '.jpg'); // on stocke la nouvelle image vers le chemin souhaité
	}
